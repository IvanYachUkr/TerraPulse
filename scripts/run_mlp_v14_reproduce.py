#!/usr/bin/env python3
"""
MLP V14: Reproduce V10 champion result.

Single config: bi_LBP (864 feat) × plain_silu_L5_d1024_bn × ILR head.
Data source:   features_merged_full.parquet (Python-extracted, same as V10).

Expected R² ≈ 0.787 ± 0.038 (V10 result).
ETA: ~25 min (5 folds × ~5 min each).
"""
...
V14_CSV = os.path.join(OUT_DIR, "mlp_v14_reproduce.csv")
...
def main():
    parser = argparse.ArgumentParser(description="V14: Reproduce V10 champion")
    ...
    # Load PYTHON features (same as V10)
    print("Loading Python features (features_merged_full.parquet)...", flush=True)
    ...
    print(f"\n{'='*70}", flush=True)
    print(f"MLP V14 — Reproduce V10 Champion", flush=True)
    print(f"{'='*70}", flush=True)
    print(f"  Config:     {name}", flush=True)
    print(f"  Features:   {n_features} (bi_LBP from Python parquet)", flush=True)
    ...
    results = []
    done_keys = set()
    if os.path.exists(V14_CSV):
        df_old = pd.read_csv(V14_CSV)
    ...
    for fold_id in folds_to_run:
        ...
        pd.DataFrame(results).to_csv(V14_CSV, index=False)
    ...
    # Final summary
    print(f"\n{'='*70}", flush=True)
    print(f"V14 RESULTS — Reproduction of V10 Champion", flush=True)
    print(f"{'='*70}", flush=True)
    print(f"  Config:     {name}", flush=True)
    print(f"  Features:   {n_features} (bi_LBP)", flush=True)
    print(f"  Data:       features_merged_full.parquet (Python)", flush=True)
