#!/usr/bin/env python3
"""
Multi-City Pipeline V3: Production model.

Train ONE production model on 14 cities, validate on Nuremberg (unseen).
Features: stratified batch sampling, longer patience, checkpoints.
...
Usage:
    .venv/Scripts/python.exe scripts/run_multi_city_pipeline_v3.py
...
MODELS_DIR = os.path.join(CITIES_DIR, "models_v3")
...
# ---------------------------------------------------------------------------
# Stratified City-Class Batch Sampler
# ---------------------------------------------------------------------------

class StratifiedCitySampler:
    """
    Yields batches balanced by dominant class AND city.

    For each batch:
      1. Sample batch_size / N_CLASSES cells per class
      2. Within each class, cycle through cities round-robin
      3. Shuffle the combined batch
    """

    def __init__(self, y, city_labels, batch_size=2048, seed=42):
        self.batch_size = batch_size
        self.rng = np.random.RandomState(seed)
        self.n = len(y)

        # Assign each cell to its dominant class
        dominant = np.argmax(y, axis=1)
        self.class_indices = {}
        self.city_class_indices = {}

        unique_cities = sorted(set(city_labels))
        for cls_id in range(N_CLASSES):
            cls_mask = dominant == cls_id
            self.class_indices[cls_id] = np.where(cls_mask)[0]
            self.city_class_indices[cls_id] = {}
            for city in unique_cities:
                city_mask = np.array([c == city for c in city_labels])
                combined = cls_mask & city_mask
                idx = np.where(combined)[0]
                if len(idx) > 0:
                    self.city_class_indices[cls_id][city] = idx

        # Print class distribution
        print(f"  StratifiedSampler: {self.n} cells, {len(unique_cities)} cities")
        for cls_id, name in enumerate(CLASS_NAMES):
            n_cls = len(self.class_indices[cls_id])
            n_cities = len(self.city_class_indices[cls_id])
            print(f"    {name:<15} {n_cls:>6} cells ({n_cities} cities)")

    def __iter__(self):
        """Yield index arrays for one epoch."""
        # Shuffle within each class-city bucket
        shuffled = {}
        for cls_id in range(N_CLASSES):
            shuffled[cls_id] = {}
            for city, idx in self.city_class_indices[cls_id].items():
                perm = self.rng.permutation(len(idx))
                shuffled[cls_id][city] = idx[perm]

        # Track position within each city bucket
        positions = {cls_id: {city: 0 for city in shuffled[cls_id]}
                     for cls_id in range(N_CLASSES)}

        per_class = max(self.batch_size // N_CLASSES, 1)
        n_batches = max(self.n // self.batch_size, 1)

        for _ in range(n_batches):
            batch_indices = []
            for cls_id in range(N_CLASSES):
                cities = list(shuffled[cls_id].keys())
                if not cities:
                    continue
                # Round-robin across cities
                sampled = 0
                city_idx = self.rng.randint(0, len(cities))
                while sampled < per_class:
                    city = cities[city_idx % len(cities)]
                    arr = shuffled[cls_id][city]
                    pos = positions[cls_id][city]
                    if pos >= len(arr):
                        # Reshuffle and reset
                        perm = self.rng.permutation(len(arr))
                        shuffled[cls_id][city] = arr[perm]
                        positions[cls_id][city] = 0
                        pos = 0
                        arr = shuffled[cls_id][city]
                    # Take up to per_class_per_city cells
                    take = min(per_class - sampled, len(arr) - pos,
                               max(per_class // len(cities), 1))
                    batch_indices.extend(arr[pos:pos + take].tolist())
                    positions[cls_id][city] = pos + take
                    sampled += take
                    city_idx += 1

            # Shuffle the batch
            self.rng.shuffle(batch_indices)
            yield np.array(batch_indices, dtype=np.int64)
...
def train_production_mlp(X_train, y_train, X_val, y_val,
    ...
    try:
        optimizer = torch.optim.AdamW(
            net.parameters(), lr=cfg["lr"], weight_decay=1e-4, fused=use_amp)
    except TypeError:
        optimizer = torch.optim.AdamW(
            net.parameters(), lr=cfg["lr"], weight_decay=1e-4)
...
def stage_train():
    ...
    if not tree_done:
        ...
        tree_meta = {
            "model": "LightGBM", "training": "production_v3",
    ...
    if not mlp_done:
        ...
        mlp_meta = {
            "model": "MLP", "training": "production_v3",
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-city V3 production pipeline")
    ...
    t_total = time.time()
    print(f"[{ts()}] Multi-City Pipeline V3 (Production) starting")
    ...
    print(f"  MLP: {MLP_DEPTH}L x {MLP_WIDTH}w, {N_SEEDS} seeds")
    ...
    print(f"\n{'='*70}")
    print(f"MULTI-CITY PIPELINE V3 COMPLETE in {hours}h {mins}m")
