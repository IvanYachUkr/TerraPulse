#!/usr/bin/env python3
"""
V7 MLP Sweep — identical to V6 but uses V7 features (Rust-downloaded,
spatial NN NaN fill). Monkey-patches city_features_dir to point to
features_v7/ instead of features/.

After the sweep completes, automatically exports the best model to ONNX
for the Rust inference pipeline.

Usage:
    .venv/Scripts/python.exe scripts/sweep_mlp_v7.py
    .venv/Scripts/python.exe scripts/sweep_mlp_v7.py --cleanup-dry-run
    .venv/Scripts/python.exe scripts/sweep_mlp_v7.py --cleanup-old
"""
...
def _city_features_dir_v7(city):
    """Override: point to features_v7/ instead of features/."""
    return os.path.join(v5_mod.city_dir(city), "features_v7")


# Patch the module-level function so sweep_mlp_v6 picks it up
v5_mod.city_features_dir = _city_features_dir_v7

V7_SWEEP_DIR = os.path.join(CITIES_DIR, "models_v7_sweep")

# Also patch the output directory
os.environ["SWEEP_OUTPUT_DIR"] = V7_SWEEP_DIR


# ---------------------------------------------------------------------------
# Optional: clean up old V5 features to free disk space
# ---------------------------------------------------------------------------
def cleanup_old_features(dry_run=True):
    """Delete features/ dir for cities that have features_v7/."""
    from scripts.run_multi_city_pipeline_v5 import CITIES
    freed = 0
    for city in CITIES:
        old_dir = os.path.join(v5_mod.city_dir(city), "features")
        new_dir = _city_features_dir_v7(city)
        new_parquet = os.path.join(new_dir, "features_rust_2020_2021.parquet")

        if os.path.exists(new_parquet) and os.path.isdir(old_dir):
            size = sum(
            ...
            if dry_run:
                print(f"  [DRY] Would delete {old_dir} ({size/1e6:.1f} MB)")
            else:
                shutil.rmtree(old_dir)
                print(f"  Deleted {old_dir} ({size/1e6:.1f} MB)")
            freed += size

    action = "Would free" if dry_run else "Freed"
    print(f"  {action} {freed/1e9:.1f} GB total")
    return freed
...
def export_best_onnx(sweep_dir):
    ...
    sep = "=" * 60
    print(f"\n{sep}")
    print("ONNX EXPORT: Best V7 Model")
    print(sep)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cleanup-old", action="store_true",
                        help="Delete old features/ dirs where features_v7/ exists")
    ...
    if args.cleanup_dry_run:
        cleanup_old_features(dry_run=True)
        sys.exit(0)

    if args.cleanup_old:
        cleanup_old_features(dry_run=False)

    if args.export_only:
        export_best_onnx(V7_SWEEP_DIR)
        sys.exit(0)

    # Run V6 sweep (with monkey-patched features dir)
    print(f"\nRunning V7 MLP Sweep (features from features_v7/)")
    print(f"Output dir: {V7_SWEEP_DIR}\n")
