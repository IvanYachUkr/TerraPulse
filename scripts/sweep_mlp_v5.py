#!/usr/bin/env python3
"""
V5 MLP Architecture Sweep: exhaustive search over constant-width and tapered
architectures. Evaluates every config on 3 test cities (Nuremberg, Frankfurt,
Munich). Saves best weights and full results.

Usage:
    .venv/Scripts/python.exe scripts/sweep_mlp_v5.py
"""
...
CONFIGS = [
    ...
    ("C_5x4096",           "C", (5, 4096),                         0.35, 1e-3),
    ("C_3x4096",           "C", (3, 4096),                         0.30, 1e-3),

    # ---- XLarge (for 50-city training, ~1.3M samples) ----
    ("C_7x2048",           "C", (7, 2048),                         0.30, 1e-3),
    ("C_4x4096",           "C", (4, 4096),                         0.35, 1e-3),
    ("C_6x3072",           "C", (6, 3072),                         0.35, 1e-3),
...
OUT_DIR = os.path.join(PROJECT_ROOT, "data", "cities", "models_v5_sweep", RUN_ID)
...
print(f"[{ts()}] V5 MLP Sweep starting (run_id={RUN_ID})")
...
# Training data stays on CPU (8.6 GB won't fit in 8 GB VRAM).
# Only each mini-batch (~28 MB) is transferred to GPU during training.
...
# Keep as numpy on CPU — batch-level GPU transfer in training loop
X_trn_np = X_train    # [N, F] float32 on CPU (already scaled in-place)
y_trn_np = y_norm     # [N, C] float32 on CPU
del y_norm

# Prepare test tensors (test data is small enough for GPU ~1 GB)
test_tensors = {}
...
for ci, (cfg_name, cfg_type, shape_params, dropout, wd) in enumerate(CONFIGS):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for start in range(0, len(perm), BATCH_SIZE):
            ...
            # CPU → GPU transfer per batch (~28 MB)
            xb = torch.tensor(X_trn_np[idx], device=device, dtype=torch.float32)
            yb = torch.tensor(y_trn_np[idx], device=device, dtype=torch.float32)
...
print(f"\n\n{'='*90}")
print(f"V5 MLP SWEEP COMPLETE ({len(CONFIGS)} configs)")
print(f"{'='*90}")
