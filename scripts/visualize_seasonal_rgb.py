"""
Visualize Sentinel-2 seasonal composites as true-color RGB PNGs.

Reads the 11-band seasonal TIFs from data/raw/v2/ (produced by the Rust
binary using 25th-percentile compositing with cloud masking), extracts
B04 (Red), B03 (Green), B02 (Blue), and renders them as natural-color
images using a 2-98 percentile stretch.

Outputs:
  - Individual PNGs per year/season  → data/raw/v2/rgb_preview/
  - Grid overview (6 years × 3 seasons) → data/raw/v2/rgb_preview/overview_grid.png
...
def tif_to_rgb(tif_path: str) -> tuple[np.ndarray, dict]:
...
def make_grid(images: dict, cell_max_w: int = 400) -> Image.Image:
    ...
    """
    # Figure out cell size (all images same size, but downsample for grid)
...
def main():
    print("=" * 60)
    print("Visualizing Seasonal Composites (Rust 25th-percentile)")
