#!/usr/bin/env python3
"""
Multi-City Pipeline: Download -> Anchor -> Labels -> Extract -> Train -> Predict.

Train on 6 European cities, predict on all 7 (Nuremberg = held-out test).
All data is persisted under data/cities/{name}/ so it never needs to be
re-downloaded or re-extracted.

Cities (training):
    Bremen, Hamburg, Düsseldorf, Leipzig, Rostock, Amsterdam

City (test):
    Nuremberg

Usage:
    python scripts/run_multi_city_pipeline.py [--skip-download] [--skip-extract]
                                               [--skip-train] [--skip-predict]
    # Use .venv python for GPU MLP training:
    .venv/Scripts/python.exe scripts/run_multi_city_pipeline.py ...

Runtime: ~8-12 hours (download-heavy, GPU training ~1h)
"""
...
MODELS_DIR = os.path.join(CITIES_DIR, "models")
...
def train_mlp_model(X_train, y_train, X_val, y_val, fold_id, n_features,
                    device, seed_offset=0):
    """Train MLP with specified seed."""
    import torch
...
def stage_train():
    ...
    # --- Train MLP (multi-seed) ---
    if not mlp_done:
        import torch
        from scripts.run_mlp_overnight_v4 import _predict_batched

        device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"\n  [{ts()}] Training MLP on {device} "
              f"({N_SEEDS} seeds x {N_FOLDS} folds = {N_SEEDS * N_FOLDS} "
              f"models)...")
        ...
        for fold_id in range(N_FOLDS):
            ...
            for seed_idx, seed_offset in enumerate(seed_offsets):
                print(f"\n  --- MLP Fold {fold_id} Seed {seed_idx} "
        ...
        print(f"  MLP ensemble mean R2: {mlp_r2:.4f}")
...
def predict_with_mlp_ensemble(X, fold_id, n_features, device):
