#!/usr/bin/env python3
"""
Build a multi-city deck.gl map for SpectralSpatialNet V7 predictions.

For each of the 6 test cities, renders:
  - SSNet V7 predictions
...
def load_ssnet_v7(device):
    from reproduce.models.architectures.spectral_spatial_v7 import SpectralSpatialNetV7
    ...
    state = torch.load(os.path.join(CKPT_DIR, "ssnet_v7.pt"),
                       map_location=device, weights_only=True)
    model.load_state_dict(state)
    model.eval()
    with open(os.path.join(CKPT_DIR, "ssnet_v7_fixed_scaler.pkl"), "rb") as f:
        sc = pickle.load(f)
    return model, sc["patches"], sc["indices"]


# ── V7 prediction (needs center_raw) ────────────────────────────────────────

def predict_v7_full(model, ps, idx_s, cube, H, W, device):
    """Predict all pixels using SSNet V7, row by row with 3×3 patches."""
...
<head>
    <meta charset="utf-8">
    <title>SSNet V7 - All Test Cities</title>
...
<div id="info" class="panel">
    <div class="model-tag">SSNet V7.1</div><br>
    <strong>Prior-guided ambiguity-aware</strong><br>
    Test accuracy: <strong>86.65%</strong><br>
    Val balanced: <strong>74.92%</strong><br>
    1.41M params · 41 epochs
</div>
...
def main():
    ...
    print(f"[{ts()}] SSNet V7 prediction map builder")
    ...
    import json
    metrics_path = os.path.join(CKPT_DIR, "ssnet_v7_metrics.json")
    per_city_acc = {}
    ...
    # Load V7 model
    print(f"\n[{ts()}] Loading SSNet V7...")
    v7_model, v7_ps, v7_is = load_ssnet_v7(device)
    print(f"  V7: {v7_model.n_params():,} params")
    ...
    for city in test_cities:
        ...
        # SSNet V7
        print(f"  [{ts()}] SSNet V7 predicting {H*W:,} pixels...")
        v7_pred = predict_v7_full(v7_model, v7_ps, v7_is, cube, H, W, device)
        v7_uri = class_map_to_png_b64(v7_pred)
        print(f"    V7 done: {len(v7_uri)//1024:,} KB")
        del v7_pred
        del cube
        gc.collect()
        ...
        cities_data.append({
            ...
            "accuracy": acc_str,
            "layers": [
                ("v7", f"SSNet V7 ({acc_str})", v7_uri),
                ("labels", "ESA WorldCover Labels", labels_uri),
            ],
        })
        del v7_uri, labels_uri
        gc.collect()
    ...
    out_path = os.path.join(OUT_DIR, "v7_predictions.html")
