#!/usr/bin/env python3
"""
Train SpectralSpatialNet V7: prior-guided ambiguity-aware classifier.

Main ideas:
  1. Keep the fast / stable V5 engineering (fixed scalers + iterative resampling).
  2. Keep the center-aware spatial readout.
  3. Add a spectral-prior head driven by raw unscaled center-pixel summaries.
  4. Add an ambiguity head so mixed / boundary pixels are not trusted blindly.
  5. Use high-precision heuristic anchors (water / built / vegetation / bare / etc.)
     to soften labels selectively, instead of letting ESA bully every pixel equally.

This is intentionally not a V5 student. It uses raw spectral structure to push beyond
pure label obedience.
"""
...
from reproduce.models.architectures.spectral_spatial_v7 import SpectralSpatialNetV7  # noqa: E402

OUT_DIR = os.path.join(PROJECT_ROOT, "reproduce", "models", "checkpoints")
VAL_CACHE_DIR = os.path.join(OUT_DIR, "val_raw_cache_v7")
FIXED_SCALER_PATH = os.path.join(OUT_DIR, "ssnet_v7_fixed_scaler.pkl")
BEST_MODEL_PATH = os.path.join(OUT_DIR, "ssnet_v7.pt")
ROUND_MODEL_PATH = os.path.join(OUT_DIR, "ssnet_v7_round.pt")
CKPT_PATH = os.path.join(OUT_DIR, "ssnet_v7_training.pt")
METRICS_PATH = os.path.join(OUT_DIR, "ssnet_v7_metrics.json")
DEFAULT_INIT_PATH = os.path.join(OUT_DIR, "ssnet_v5.pt")
DEFAULT_V5_SCALER_PATH = os.path.join(OUT_DIR, "ssnet_v5_fixed_scaler.pkl")
...
def load_or_fit_fixed_scalers(train_cities):
    ...
    if os.path.exists(DEFAULT_V5_SCALER_PATH):
        print(f"\n[{ts()}] Reusing V5 fixed scalers from {DEFAULT_V5_SCALER_PATH}...")
        with open(DEFAULT_V5_SCALER_PATH, "rb") as f:
            sc = pickle.load(f)
...
def scale_val_to_cpu(patch_scaler, idx_scaler):
    """Load and scale val data, but keep on CPU to avoid VRAM pressure."""
...
def metrics_from_confusion(conf):
    ...
    focus_score = 0.70 * score + 0.30 * hard_recall
...
def validate(model, val_patches, val_indices, val_center, val_y_gpu, n_val, n_classes, use_amp, device):
    """Validate by streaming CPU val data to GPU in batches (avoids permanent VRAM use)."""
    model.eval()
    loss_sum = 0.0
    conf = np.zeros((n_classes, n_classes), dtype=np.int64)
    vb = 16384  # V7.1: reduced from 32768 for smaller GPU footprint
...
def maybe_load_initial_weights(model: nn.Module, init_path: Optional[str], device: str) -> int:
...
class PriorGuidedCriterion:
    def __init__(self,
                 n_classes: int,
                 class_names,
                 label_smoothing: float = 0.02,
                 anchor_conf_thresh: float = 0.78,
                 agree_alpha: float = 0.08,
                 disagree_alpha_max: float = 0.55,
                 min_weight: float = 0.35,
                 boundary_downweight: float = 0.35,
                 ambiguity_downweight: float = 0.25,
                 trusted_boundary_max: float = 0.55,
                 trusted_ambiguity_max: float = 0.55,
                 prior_aux_weight: float = 0.12,
    ...
    def __call__(self, outputs: Dict[str, torch.Tensor], y: torch.Tensor):
        ...
        agree = anchor_mask & anchor_pred.eq(y)
        disagree = anchor_mask & (~anchor_pred.eq(y))

        hard_target = self._smoothed_one_hot(y)
        alpha = torch.zeros_like(anchor_conf)
        alpha[agree] = self.agree_alpha * anchor_conf[agree]
        alpha[disagree] = self.disagree_alpha_max * anchor_conf[disagree] * (0.40 + 0.60 * ambiguity[disagree])
        soft_target = hard_target * (1.0 - alpha.unsqueeze(1)) + anchor_probs * alpha.unsqueeze(1)

        sample_weight = 1.0 - self.boundary_downweight * boundary - self.ambiguity_downweight * ambiguity
        sample_weight[disagree] = sample_weight[disagree] * (1.0 - 0.30 * anchor_conf[disagree])
        sample_weight = sample_weight.clamp(min=self.min_weight, max=1.0)

        main = self._soft_ce(logits, soft_target, weight=sample_weight)

        trusted_mask = (boundary < self.trusted_boundary_max) & (ambiguity < self.trusted_ambiguity_max)
        ...
        ambiguity_target = torch.clamp(0.65 * boundary + 0.35 * (disagree.float() * anchor_conf), 0.0, 1.0)
        ambiguity_aux = F.binary_cross_entropy_with_logits(outputs["ambiguity_logit"], ambiguity_target)

        gate_target = torch.clamp(0.75 * (anchor_mask.float() * anchor_conf) + 0.25 * boundary, 0.0, 1.0)
        gate_aux = F.binary_cross_entropy_with_logits(outputs["prior_gate_logit"], gate_target)
        ...
        stats = {
            ...
            "alpha_nz": float((alpha > 1e-8).float().mean().detach().cpu()),
            "anchor_conf": float(anchor_conf.mean().detach().cpu()),
            "anchor_frac": float(anchor_mask.float().mean().detach().cpu()),
            "relabel_frac": float(disagree.float().mean().detach().cpu()),
...
def train(device, n_rounds, max_epochs_per_round, batch_size, lr,
          ...
          ckpt_metric="score", init_path: Optional[str] = DEFAULT_INIT_PATH):
    train_cities = get_train_cities()
    val_cities = get_val_cities()

    print(f"\n{'='*84}")
    print("  SpectralSpatialNet V7 — Prior-guided ambiguity-aware training")
    ...
    criterion = PriorGuidedCriterion(
        n_classes=N_CLASSES,
        class_names=CLASS_NAMES,
        label_smoothing=0.02,
        anchor_conf_thresh=0.90,        # V7.1: raised from 0.78 (target anc_frac ~0.10-0.18)
        agree_alpha=0.04,               # V7.1: halved from 0.08
        disagree_alpha_max=0.20,         # V7.1: cut from 0.55 (target alpha ~0.01-0.03)
        min_weight=0.35,
        boundary_downweight=0.35,
        ambiguity_downweight=0.25,
        trusted_boundary_max=0.55,
        trusted_ambiguity_max=0.55,
        prior_aux_weight=0.06,           # V7.1: halved from 0.12
    ...
    for round_idx in range(start_round, n_rounds):
        ...
        # V7.1: pre-cast center_raw to float32 ONCE instead of per-batch .astype()
        ...
        for epoch in range(max_epochs_per_round):
            ...
            # --- Epoch timing and memory diagnostics ---
            ...
            if torch.cuda.is_available():
                ...
                torch.cuda.reset_peak_memory_stats()
                # Check nvidia-smi for shared GPU memory (Windows-specific spill detection)
                try:
                    import subprocess
                    smi = subprocess.run(
...
def main():
    parser = argparse.ArgumentParser(description="Train SpectralSpatialNet V7")
    ...
    parser.add_argument("--init-path", type=str, default=DEFAULT_INIT_PATH,
                        help="Optional checkpoint to warm-start shared V5-compatible weights")
    ...
    if result:
        print(f"\n{'='*84}")
        print("  DONE: SpectralSpatialNet V7")
